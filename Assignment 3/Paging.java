...
public class Paging {
	...
	public int replacePageLRU(int replacePageNum) {
		...
		for(int i = 0; i < numVirtualPages; i++){
			if (pageTable[i].lastTouchTime > usedLongAgo){
				pageToReplace = i;
				usedLongAgo = pageTable[i].lastTouchTime;
			}
		}
		freedFrame = pageTable[pageToReplace].frame; 
		updatePageTableEntry(pageToReplace, -1, (byte) 0, (byte) 0, (byte) 0,
							 (byte) 0, 0, 0);
		...
		logReplacement(pageToReplace, replacePageNum, freedFrame);
	...
	public int replacePageCLOCK(int replacePageNum) {
		...
		while(!flag){
			if(pageTable[pageBuffer[bufPointer]].u == 1){
				...
				bufPointer++;
				if(bufPointer == numPhysicalPages){
					bufPointer = 0;
				}
			}else{
